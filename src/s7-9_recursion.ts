...
const factIt = (num: number): number => {
  let sum = 1;
  for (let i = 1; i <= num; i++) {
    sum *= i;
  }
...
const collectAllOdds = (arr: number[]): number[] => {
  let list: number[] = [];

  const helper = (newArr: number[]): number[] | undefined => {
    if (newArr.length === 0) return;

    const num = newArr.pop() as number
...
const powerIt = (base: number, exp: number): number => {
  let answer = 1;
  for (let i = 0; i < exp; i++) {
    answer*=base
  }
...
const findFibNum= (position: number): number => {
...
const buildFib = (position: number): number => {
  let arr = [1, 1]
...
const reverseStrArray = (str: string): string => {
  return str.split('').reverse().join('')
}

const reverseStrIt = (str: string): string => {
...
const reverseStrRec = (str: string): string => {
...
function reverseEZ(str: string): string {
...
function flatten(arr: any): any[] {
  ...
  const helper = (array: any): any => {
    if (!Array.isArray(array)) return answer.push(array)
...
function stringify(object: any): any {
  return Object.fromEntries(Object
      .entries(object)
      .map(([k, v]) => {
        if (typeof v === 'number') { return [k, v.toString()] }
        if (Array.isArray(v)) { return [k, v.map(stringify)] }
        if (typeof v === 'object') { return [k, stringify(v)] }
        return [k, v]
      })
  )
}

const stringify2 = (object: any): any => {
  let newObj: any = {}

  const helper = (obj: any) => {
    if (Array.isArray(obj)) return obj.forEach(helper)

    for (let k in obj) {
      if (typeof obj[k] === 'number') { newObj[k] = obj[k].toString(); }
      else if (typeof obj[k] === 'object') { newObj[k] = stringify2(obj[k]); }
      else { newObj[k] = obj[k]; }
    }
  }
  helper(object)
...
const collectStrings = (obj: any): string[] => {
  ...
  const helper = (val: any) => {
    if (typeof val === "string") { answer.push(val); return }
