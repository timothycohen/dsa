const indexOf = (arr: string[], val: string): number => {
...
const binaryIndexOf = (arr: string[], val: string): number => {
...
const naiveStringSearch = (str: string, sub: string): number => {
  let count = 0;
...
const buildKNPTable = (searchStr: string): number[] => {
  ...
  for (let j = 1; j < searchStr.length; j++) {
    if (searchStr[i] === searchStr[j]) { table[j] = i+1; i++; continue; }
    if (searchStr[i] !== searchStr[j]) { table[j] = table[i-1] || 0 }
  }
  return table
}
