...
// easiest solution
const areThereDuplicatesEasy = (...args: any[]) => {
...
const areThereDuplicatesF = <T extends string | number>(...args: T[]): boolean => {
  let counter = {} as Record<T, number>
...
const averagePair = (sArr: number[], targetAvg: number): boolean => {
...
const maxSubarraySum3 = (arr: number[], n: number): number | null => {
...
const minSubarrayLen = (arr: number[], target: number): number | null => {
