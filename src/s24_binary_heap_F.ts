...
export const createNumBinaryHeap = () => {
...
export const createPQNode = <T extends Primitive>(value: T, priority?: number) => ({
  value,
  priority: priority ?? 5,
});

export const createPriorityQueue = <T extends Primitive>() => {
  // higher priority has a lower priority number
  const isGreater = (n1: PQNode<T>, n2: PQNode<T>): boolean => n1?.priority < n2?.priority;
  const isEqual = (n1: PQNode<T>, n2: PQNode<T>): boolean =>
    n1?.priority === n2?.priority && n1?.value === n2?.value;
  return createBH(isEqual, isGreater);
};

export const createFIFOPQNode = <T extends Primitive>(value: T, priority: number) => ({
  value,
  priority: priority ?? 5,
  creationTime: Date.now(),
});

export const createFIFOPriorityQueue = <T extends Primitive>() => {
